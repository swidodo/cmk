        ...
        <div class="container justify-content-center">
            <div class="col-lg-12">
                <div class="card shadow-sm border-1 border-dark rounded-lg">
                    ...
                    <div class="card-body">
                    <form id="form_create_application" method="POST" action="/application/{{ $app->id }}">
                        @csrf
                        @method('PUT')
                            <div class="row gx-3">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input class="form-control @error('name') is-invalid @enderror" id="inputapliksi" type="text" name="name" value="{{ $app->name, old('name') }}" placeholder="Input nama aplikasi" />
                                        ...
                                        <select class="form-select @error('group_application_id') is-invalid @enderror" name="group_application_id" id="group_application_id">
                                            @foreach ($group as $groups)
                                                <option value="{{$groups->id}}"
                                                    @if (old('group_application_id')=='')
                                                    {{($groups->id == $app->group_application_id) ? 'selected':'' }}
                                                    @else
                                                    {{($groups->id == old('group_application_id')) ? 'selected':'' }}
                                                    @endif
                                                >
                                                    {{ $groups->group_name }}
                                                </option>
                            ...
                            <div class="row gx-3">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input class="form-control @error('directory') is-invalid @enderror" id="directory" type="text" name="directory" value="{{ $app->directory, old('directory') }}" placeholder="Input folder" />
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" placeholder="Input deskripsi">{{ $app->description, old('description') }}</textarea>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control form-select @error('status') is-invalid @enderror" name="status" id="status">
                                            <option value="1" @if (old('status') == '1' | $app->status == '1') selected="selected" @endif>Aktif {{ old('status') }}</option>
                                            <option value="0"  @if (old('status') == '0' | $app->status == '0') selected="selected" @endif>Non Aktif</option>
                                        </select>
